...
(use-feature! hideshow
  ...
  ;; Display the number of hidden lines next to the ellipsis.
  (setopt hs-display-lines-hidden t)

  ;; Display block hide/show indicators on the fringe.
  (setopt hs-show-indicators t)

  ;; `TAB' cycles the visibility anywhere on a headline.
  (setopt hs-cycle-filter t))
...
(use-feature! c-ts-mode
  ...
  (defhook! my--c-ts-mode-indent-style ()
    (c-ts-mode-hook c++-ts-mode-hook)
    "Extend the built-in BSD indentation style with additional rules."
    (let ((language (if (eq major-mode 'c-ts-mode) 'c 'cpp)))
      (setf (alist-get language treesit-simple-indent-rules)
            (append
             `(
               ...
               ;; Do not indent inside C++ namespaces.
               ,@(when (eq language 'cpp)
                   '(((n-p-gp nil nil "namespace_definition") grand-parent 0))))
             (alist-get language treesit-simple-indent-rules)))))

  ;; Start with BSD rules, the mode hook above prepends local overrides.
...
(use-package! gptel
  ...
  ;; Define only a subset of OpenAI models.
  (defconst my--gptel-openai-models
    '((gpt-6-luna
  ...
  ;; Define only a subset of Anthropic models.
  (defconst my--gptel-anthropic-models
    '((claude-opus-5.5
  ...
  (let ((backend-openai (gptel-make-openai-oauth "OpenAI"
                          :models my--gptel-openai-models))
        (backend-copilot (gptel-make-gh-copilot "Copilot"
                           :models (append my--gptel-openai-models
                                           my--gptel-anthropic-models))))
    (setopt gptel-backend backend-copilot))

  ;; Set GPT-6 Sol as the default model.
  (setopt gptel-model 'gpt-6-sol)
...
(use-package! majutsu
  ...
  (defhook! my--majutsu-jjdescription-mode-setup ()
    ...
    (display-fill-column-indicator-mode +1)
    (majutsu-jjdescription-insert-ticket-name))
