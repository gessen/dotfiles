...
abbr -a gdn git -c pager.diff=diffnav diff
...
abbr -a ja jj arrange

abbr -a jaba jj abandon

abbr -a jabs jj absorb
abbr -a jabsi jj absorb --interactive
...
abbr -a jrsi jj restore --interactive
...
function ew-devenv-start -d "Start dev container"
    ...
    docker run \
        ...
        --mount type=bind,src=$HOME/Projects,target=$HOME/Projects \
        --env SSH_AUTH_SOCK \
        --network host \
